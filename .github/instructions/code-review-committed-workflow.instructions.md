---
description: "Shared committed-review workflow used by the VS Code prompt and CLI committed-review agent."
---

# Shared Committed Review Workflow

This file defines the shared execution workflow for committed code review.

Use it as the workflow layer for:

- `.github/prompts/code-review-committed-changes.prompt.md`
- `plugins/terraform-azurerm-ai-toolkit/agents/review-committed.agent.md`

This file is not the normative rules contract.

- `.github/instructions/code-review-compliance-contract.instructions.md` remains authoritative for review rules, evidence hierarchy, finding classification, scope rules, and azurerm-linter policy.
- This file is authoritative for the committed-review execution guardrails, mandatory procedure, and output contract.
...
- any surface-specific input mapping such as explicit `repo_path`, `pr_number`, or `revision_range`
...
Assume the user may invoke the workflow with minimal instructions. Run the full procedure below even if the request is short.

### Fresh-run requirement

Every invocation of this workflow is a new audit run.
Do not reuse prior git output, linter output, file classifications, or review conclusions from earlier turns.
If the user asks to run the review again, rerun the full mandatory procedure from step 0 using the current workspace state.

### No cached review state

A previous review in the conversation is not evidence for the current run.
All review findings must be based on commands and file reads executed during the current invocation of this workflow.
...
If this invocation completes the mandatory procedure successfully, emit the full current review template defined by this workflow.
...
The required git and `azurerm-linter` commands in this workflow are already authorized by the workflow itself.
...
- Follow the shared review contract, not stale memory.
...
- The only allowed normal output is the review template defined in this workflow.
...
- Do not output bookkeeping or completion-status narration such as `I have the findings`, `I am filling in the review bookkeeping`, `I am doing the last guidance reads`, `I am resolving the counts`, or similar.
...
- If the contract is not fully loaded, hard-stop and output exactly this one line and nothing else:
  - `Cannot run code-review-committed-changes: code review contract not fully loaded. Load .github/instructions/code-review-compliance-contract.instructions.md to EOF and re-run this workflow.`
...
The commands in steps 1 and 4 must be executed again for each invocation of this workflow, even if they were executed earlier in the conversation.
...
- When GitHub-native pull request tools or environment-provided pull request metadata are available, prefer them over generic URL fetches.
- Do not fetch raw `.diff` or `.patch` URLs, and do not call raw `api.github.com` pull-request endpoints through generic web-fetch tooling, when GitHub-native pull request tools can provide the required PR metadata or changed-file set.
- Do not run ad hoc shell, PowerShell, Python, jq, regex, or similar probe scripts to inspect, enumerate, or reshape external PR payloads when GitHub-native PR tools or environment PR metadata are available.
- If authoritative PR metadata exists but is inconvenient to search through workspace tools, do not compensate by running shell probes against external PR payloads; use GitHub-native PR tools or the already-provided authoritative metadata silently.
- Perform PR-scope source selection silently. Do not output user-facing narration such as `switching to direct GitHub PR APIs`, `pulling the PR files list`, or similar source-selection commentary before or during the review.
...
- When authoritative PR scope already exists, do not run extra repo-local shell commands or ad hoc PowerShell bookkeeping scripts solely to compute summary fields such as branch name, added/modified/deleted counts, vendored-file counts, or insertion/deletion counts.
- When authoritative PR metadata or diff evidence already provides the needed summary information, reuse that evidence silently instead of running extra bookkeeping commands.
- If a non-essential bookkeeping field cannot be determined without additional shell approval or ad hoc scripting, render a concise `PR scope` or `n/a` value and continue the review rather than interrupting the user for bookkeeping-only commands.
- If authoritative PR scope cannot be interpreted without ad hoc shell probing of external PR payloads, hard-stop and output exactly this one line and nothing else:
  - `Cannot run code-review-committed-changes: authoritative PR scope requires ad hoc probing of external PR payloads. Re-run with GitHub-native PR context or environment-provided PR metadata instead.`
...
- If the review scope includes `internal/**/*.go`, explicitly read these implementation-side guidance files before producing findings:
...
- For Go files, treat the implementation guide's `Copyright Header (Required)` section as mandatory evidence, including the exact required copyright and SPDX header text.
- If the review scope includes `internal/**/*_test.go`, explicitly read these testing-side guidance files before producing findings:
...
## 📊 **CHANGE SUMMARY**
- **Files Changed**: [number] files ([tracked_additions] new, [modifications] modified, [deletions] deleted, when deterministically available)
- **Scale**: [insertions] insertions, [deletions] deletions, or `n/a` when non-essential bookkeeping data is unavailable without extra execution
- **Branch**: [current_branch] vs origin/main, `PR scope`, or `n/a`
...
- **File Skipped**: [active entrypoint file path] - Cannot review the active committed-review entrypoint itself to prevent infinite loops
