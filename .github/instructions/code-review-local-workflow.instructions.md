---
description: "Shared local-review workflow used by the VS Code prompt and CLI local-review agent."
---

# Shared Local Review Workflow

This file defines the shared execution workflow for local code review.

Use it as the workflow layer for:

- `.github/prompts/code-review-local-changes.prompt.md`
- `plugins/terraform-azurerm-ai-toolkit/agents/review-local.agent.md`
...
- This file is authoritative for the local-review execution guardrails, mandatory procedure, and output contract.
...
- any surface-specific input mapping such as explicit `repo_path` or `diff_scope`
...
- The only allowed normal output is the review template defined by this workflow.
...
- If the contract is not fully loaded, hard-stop and output exactly this one line and nothing else:
  - `Cannot run code-review-local-changes: code review contract not fully loaded. Load .github/instructions/code-review-compliance-contract.instructions.md to EOF and re-run this workflow.`
...
- If `diff_scope` is `staged`, review the staged diff only.
- If `diff_scope` is omitted or set to `worktree`, use the unstaged diff as the primary review scope when it is non-empty.
...
- **File Skipped**: [active entrypoint file path] - Cannot review the active local-review entrypoint itself to prevent infinite loops
