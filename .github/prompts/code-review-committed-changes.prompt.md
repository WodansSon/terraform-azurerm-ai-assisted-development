...
- When `website/docs/**/*.html.markdown` files are in scope, explicitly inspect example blocks for `DOCS-EX-000`, `DOCS-EX-010`, and `DOCS-EX-011` style failures when current-run schema, implementation, or nearby test-helper evidence proves the mismatch, and preserve those evidence-backed docs concerns even when implementation issues already exist.
