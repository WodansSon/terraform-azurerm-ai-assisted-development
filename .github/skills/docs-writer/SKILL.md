...
- Bullet split trigger (mandatory; prevents false positives and misses): evaluate concision by content, not sentence count. Keep the field definition and applicable standard `Possible values`, range/constraint, `Defaults to`, and ForceNew sentences in the bullet. In resource docs, split only identified supplemental caveats, setup instructions, rationale, examples, external guidance, conditional requirements, or extended explanation into an inline note per `DOCS-ARG-011` + `DOCS-NOTE-003`; in data source docs, keep the bullet short and field-definitional instead of adding a field-level note.
