...
- Because the Hosted and Interactive Toolkits can contain customization files with the same relative names, resolve every Hosted runtime read, edit, and validation path through its fully qualified `hosted_copilot/**` source path. Never substitute or modify a same-named root `.github/**` asset, verify the complete repository-relative path before every edit, and stop for clarification when the requested product ownership is ambiguous.
