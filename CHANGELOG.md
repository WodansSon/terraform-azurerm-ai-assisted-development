...
- **Maintainer/Workflow:**
  ...
  - **[Implementation]** - Tightened implementation guidance around provider-specific edge cases, including error-format anchors, enum-pointer boundaries, and a forward-consistency rule for validation-function placement so new or materially updated bespoke schema validators move toward service-local `validate/<subject>.go` helpers with matching unit tests without forcing churn-only cleanup of untouched legacy layouts.
  - **[Implementation]** - Generalized broad rule examples across the runtime guidance to placeholder-based patterns so the AI does not anchor on service-specific examples when applying provider-wide implementation and testing rules.
  - **[Implementation]** - Aligned the runtime implementation guidance, schema-pattern notes, testing evidence references, and user-facing examples with the current upstream contributor-doc wording that Resource Identity is mandatory for all new resources and list-resource omissions must use the documented maintainer-reviewed exception path.
  ...
  - **[Testing]** - Clarified acceptance-test config-helper style so one-use helper results like `r.template(data)` or `r.basic(data)` should be passed directly into `fmt.Sprintf(...)` instead of being assigned to temporary locals that are only forwarded once, aligning that rule to verified maintainer review guidance.
  - **[Testing]** - Added an explicit acceptance-test rule that data source tests should prefer the associated resource `complete(data)` helper by default when it exists, while still allowing narrower or scenario-specific helpers when the test setup warrants them.
  - **[Testing]** - Added an acceptance-test helper-struct naming rule that preserves one canonical helper type per Terraform surface across all test variants and generated identity tests, while preferring `(x)Resource` and `(x)DataSource` only for new surfaces that do not already have an established helper type.
  ...
  - **[Internal]** - Added adjudicated implementation-guidance regression coverage for the ValidateFunc placement caveat, so the corpus now benchmarks all three paths: untouched legacy no-churn, changed bespoke validators that should migrate to `validate/<subject>.go`, and brand-new bespoke validators that should start there with matching unit tests.
  - **[Internal]** - Added an adjudicated acceptance-testing regression case for helper-struct naming drift between acceptance tests and generated identity tests, using the Durable Task failure mode so the new generator-alignment rule is benchmarked instead of remaining prose-only.
  - **[Internal]** - Added adjudicated example results for the remaining committed-review hardening cases and a direct acceptance-testing regression case for one-use `fmt.Sprintf(...)` helper composition, so those behaviors now score in the normal regression corpus instead of existing as prose-only or unscored case manifests.
  - **[Internal]** - Added adjudicated acceptance-testing regression coverage for data source setup helper selection, so the guidance now benchmarks preferring `complete(data)` by default while still allowing justified narrower setup helpers.
