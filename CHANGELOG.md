...
- **User-Priority:**
  - **[Review]** - Local and committed review now require the audit to finish and the findings set to be frozen before the first review heading is emitted, reducing mid-stream self-corrections and appended findings during streamed review output.
  - **[Review]** - Committed review now uses GitHub-backed PR scope by default, forbids local cache or spill-file recovery, keeps GitHub CLI fallback opt-in only, and stays audit-only instead of inventing tests, helper scripts, or other prerequisite work for straightforward checks.
...
- **Maintainer/Workflow:**
  - **[Implementation]** - Moved the implementation-session workflow into the `resource-implementation` skill and tightened it around implementation-model selection, framework companion targets, generic placeholder examples, pointer and enum edge cases, validator placement, and upstream-backed list-resource and resource-identity expectations.
  - **[Testing]** - Moved acceptance-test execution workflow into the `acceptance-testing` skill and tightened the deterministic test-shape rules for inline `fmt.Sprintf(...)` helper usage, canonical helper-type naming, and preferring associated-resource `complete(data)` setup for data source tests when appropriate.
  ...
  - **[Internal]** - Added adjudicated regression coverage for the determinism hardening on this branch, including ValidateFunc placement, helper-struct naming, inline `fmt.Sprintf(...)` helper composition, data source setup helper selection, and the committed-review failure modes around singleton no-list handling, PR-scope recovery, CLI fallback, test execution, and helper-script invention.
