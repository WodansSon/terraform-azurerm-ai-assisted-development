...
- **User-Priority:**
  ...
  - **[Review]** - Variant-constrained ownership-boundary admission and lifecycle-symmetry defects are now treated as blocking by definition when current-run evidence proves foreign variants can be admitted or destroyed through generic paths, and missing `complete` coverage for broader new-resource shapes must remain as its own acceptance-matrix finding instead of being absorbed by unrelated test or metadata issues.
  - **[Review]** - The routed review roles now operate on a simpler immutable findings model: reviewer, architect, and skeptic emit visible `issue` or `observation` records; advocate adds defense commentary to those same records; moderator decides only duplicate merge, final visibility, and final issue-versus-observation classification before presentation renders the result.
  - **[Review]** - The review workflow now uses simpler immutable finding records with final `classification` and `visible` decisions instead of the previous candidate/confirmed/downgraded/dismissed plus render-section state machine, so later roles comment on the full findings set instead of filtering findings through multiple status transitions.
  - **[Review]** - Variant-constrained review guidance now treats generic ID resolution followed by only later discriminator checks as affirmative ownership-boundary evidence, and requires delete to mirror those guards or surface a lifecycle-symmetry finding even without a dedicated foreign-variant test fixture.
  - **[Review]** - Docs example correctness now explicitly requires same-argument HCL key spelling/casing parity between reference docs and local acceptance-test-backed or implementation-backed examples, so mismatches like differently cased map keys cannot hide behind generic schema-validity checks.
  - **[Review]** - Generic code review now treats acceptance-test-backed or implementation-backed Terraform HCL as sufficient local evidence for docs example field-key, map-key, and casing parity, so user-facing docs/example mismatches do not need separate API-layer proof before they can surface as docs-example correctness findings.
  - **[Review]** - Docs example correctness now explicitly covers field-key and map-key casing against local implementation-backed or acceptance-test-backed example evidence, reducing the chance that provider docs drift from tested user-facing HCL while still passing generic schema validation.
  - **[Review]** - Deterministic overlap routing now treats unchanged sibling surfaces that share the same ARM ID type or SDK remote-object method family as mandatory explicit matrix rows for variant-constrained resources, reducing the chance that generic sibling resources disappear from first-pass ownership and lifecycle review.
  - **[Review]** - The review coordinator and primary reviewer guidance now call out the exact structured `issueClasses` tokens for ownership, lifecycle symmetry, PATCH-shape, optional-state drift, acceptance-matrix, and docs-example concerns, reducing the chance that valid findings disappear because the runtime serialized them under prose-only labels instead of the schema-backed identifiers.
  - **[Review]** - The review workflow schemas now model docs-example correctness and upstream minimum acceptance-matrix coverage as explicit issue classes, so the router and handoff pipeline can preserve those concerns mechanically instead of letting them disappear as prose-only expectations.
  - **[Review]** - The primary `review-reviewer` workflow now explicitly inspects importer/read ownership first, then update/delete lifecycle symmetry, and preserves separate docs-example correctness records before secondary metadata or test-shape concerns can crowd them out on mixed discriminator-specific reviews.
  - **[Review]** - The review workflow now explicitly preserves separate lifecycle-symmetry and docs-example correctness findings alongside ownership and implementation issues on mixed discriminator-specific reviews, reducing the chance that a weaker metadata or test-shape result crowds out a real docs or lifecycle defect.
  - **[Review]** - The committed review workflow now explicitly requires a JSON-returning PR-files request shape and forbids treating terminal spill banners or saved-output wrapper text as if they were the GitHub payload, reducing false "raw output wasn't JSON" failures during authoritative PR-scope resolution.
  - **[Review]** - The shipped `review-reviewer` skill now explicitly performs ownership-boundary and lifecycle-symmetry checks first on variant-constrained managed surfaces and preserves separate mandatory issue-class findings when the evidence supports more than one concern on the same PR, reducing the chance that secondary metadata or test-shape findings crowd out real ownership bugs.
  - **[Review]** - The committed review workflow now distinguishes forbidden pre-authority spill-file scope from allowed current-run transient transport buffers, so saved artifacts are still blocked as the starting PR-scope source of truth without forbidding temporary larger-data transport after authoritative PR scope has already been established.
  - **[Review]** - The generic local and committed review workflows now route their concrete primary audit through a dedicated shipped `review-reviewer` skill, so Step 5 reviewer behavior no longer lives as a large prompt-embedded procedure and the prompts can stay closer to pure orchestration.
  - **[Review]** - The shared review presentation layer now renders `Suggested Code` blocks with the finding's source-language fence instead of GitHub `suggestion` fences, preserving syntax highlighting in non-GitHub surfaces.
  - **[Review]** - The local and committed generic review workflows now always route successful runs through `review-moderator`, including zero-findings paths, so empty candidate sets still freeze as deterministic final results instead of skipping final moderation.
  - **[Review]** - The shared review workflow now treats PATCH-versus-residual-state risks and omitted-config read-state drift on `Optional` or `Optional+Computed` API-returned fields as mandatory issue-class checks for changed provider resources, so repeated reviews of the same PR are less likely to swap one real blocking defect for another or silently skip an evidence-backed lifecycle concern.
  - **[Review]** - The generic review workflows now require every evidence-backed concern found during a mandatory issue-class check to be serialized into the shared finding set at the justified classification, so a first blocking issue can no longer silently crowd out a second independent concern from the same review.
  - **[Review]** - The generic local and committed review workflows now use a coordinator-owned post-review linkage-validation phase to fail closed before routed roles begin if the reviewer discovered an evidence-backed concern but did not serialize it into the shared handoff record set.
  - **[Review]** - The generic local and committed review workflows now require mandatory issue-class checks to emit and link handoff records immediately when they discover evidence-backed concerns, reducing the gap between discovery and routed-role adjudication.
  - **[Review]** - New or materially changed variant-constrained ownership surfaces now prioritize ownership-boundary and lifecycle-symmetry checks before secondary polish findings, and preserve separate ownership, lifecycle, PATCH-shape, and optional-state-drift findings when the evidence supports all four on the same PR.
  - **[Review]** - The generic local and committed review workflows now explicitly keep evidence-backed non-blocking concerns from mandatory issue-class checks in `OBSERVATIONS`, so a blocking issue no longer suppresses a separate atypical lifecycle concern that still matters to reviewers.
  - **[Review]** - The generic review workflow now classifies risk-only mandatory issue-class concerns as `OBSERVATIONS` unless the current run proves concrete blocking harm, and it validates file-reference policy against the assistant-emitted markdown body rather than post-render VS Code link rewriting.
  - **[Review]** - The generic review workflow now treats the upstream minimum new-resource acceptance-test matrix as `basic`, `requiresImport`, `complete`, and `update`, and preserves missing minimum scenarios as non-blocking review concerns through the mandatory issue-class workflow unless the current run proves that a specific expectation does not apply.
  - **[Review]** - The generic local and committed review workflows now preflight all routed-review and review-presentation skill and contract dependencies before starting the audit, so missing workflow assets fail fast instead of hard-stopping after findings are already frozen.
  - **[Review]** - Routed review skills now end with explicit skill EOF markers again, and the early prerequisite hard-stop now tells users that the installed workflow is incomplete or stale and names the exact files that must be present and readable end-to-end.
  - **[Review]** - The committed review workflow now fails early with a specific authoritative-PR file-availability error when a non-deleted changed file is missing from the local committed checkout, instead of surfacing that condition later as a generic coverage-matrix completion failure.
  - **[Review]** - Mixed implementation-plus-reference-doc reviews now treat evidence-backed docs example correctness under exact `DOCS-*` rules as part of the mandatory preserved concern set, so broken changed examples are less likely to disappear behind larger Go findings.
  - **[Review]** - The generic local and committed review workflows now fail closed when final findings are not presentation-complete or the rendered body does not exactly match the current presentation contract, instead of silently falling back to older layouts or incomplete issue cards.
  - **[Review]** - The generic local and committed review workflows now require the final review title to summarize what the change actually does instead of collapsing to a bare scope label such as only a PR number or `Local Changes` when richer current-run evidence exists.
  - **[Review]** - The shared review presentation layer now renders structured issues, observations, and recommendation findings as heading-based finding blocks with explicit top-level `Impact`, `Evidence`, and `Suggested Change` bullets, instead of embedding multi-part content inside a single parent bullet.
  ...
  - **[Review]** - The generic local and committed review workflows now require an explicit current-run EOF load of `review-presentation-input.schema.json` before the render-only presentation step, reducing false hard-stops caused by treating the schema as implicitly loaded through the presentation skill or contract text.
  - **[Review]** - The generic local and committed review workflows now build a deterministic coverage matrix through a shared `review-coordinator` skill before findings are drafted, forcing fixed lifecycle-window inspection, overlap-surface scans for new resources, and stable issue-class checks instead of letting the active editor file choose the first review anchor.
  - **[Review]** - The deterministic routing layer now uses a schema-backed internal coverage matrix, requires unchanged overlap surfaces to be materialized as explicit file-path rows, and blocks architect, skeptic, advocate, and moderator routing until that matrix is complete.
  - **[Review]** - The deterministic routing flow now explicitly loads the coverage-matrix schema and separates matrix build from post-standards completion validation, so standards-dependent issue-class checks can complete without reintroducing routing drift.
  - **[Review]** - The coverage-matrix model now tracks not-applicable issue classes explicitly at both the row and matrix levels, so completion can be determined from structured state instead of relying on prose-only interpretation.
  - **[Review]** - The router now defines an explicit validation sub-phase as the canonical completion gate for required rows, window coverage, issue-class coverage, overlap-row materialization, and evidence-backed completion status before findings or routed roles can proceed.

- **Maintainer/Workflow:**
  - **[Internal]** - Added the shipped `review-reviewer` runtime skill, wired it into the installer manifest, and thinned the generic local and committed review prompts so primary review execution now lives in a reusable runtime skill rather than duplicated Step 5 prose.
  - **[Internal]** - The repo-only changelog maintenance skill now explicitly treats `Unreleased` as release notes rather than commit history, requiring overlapping implementation-step bullets to be collapsed into final outcome-focused entries before validation.
  ...
  - **[Internal]** - Expanded the shipped review-coordinator skill and review-coverage-matrix schema with explicit PATCH or residual-state and optional-state-drift issue classes, so the deterministic coverage gate can mechanically require both checks instead of relying on prompt text alone.
  - **[Internal]** - Expanded the review coverage matrix with emitted handoff-record linkage fields and moved the pre-routing serialization check under coordinator-owned linkage validation, so missing reviewer-to-handoff records now hard-stop the run through workflow machinery instead of prompt-only bookkeeping.
  - **[Internal]** - Trimmed the coordinator and generic review prompts down to contract-rule references for immediate emission and linkage validation, and added contract-validation checks that those consumers still cite the authoritative rule IDs.
  - **[Internal]** - Added a committed-review regression case for the combined PATCH-behavior plus omitted-config state-drift resource pattern, so the adjudicated corpus now protects multi-issue coverage for this deterministic-issue-class gap.
  - **[Internal]** - Added a discriminator-specific first-cold-review regression benchmark and extended contract validation so the review consumers must keep referencing the contract-owned ownership-first rule family as this workflow evolves.
  - **[Internal]** - The one-shot toolkit validator now fails when the current branch changes a regression case or fixture without also making the matching case adjudicated and runnable via a `.result.json` example artifact, closing the gap where schema-valid but unscored cases looked validated.
  - **[Internal]** - The regression-runnability gate now defaults to current-worktree scope when local edits exist and falls back to branch-diff scope only on clean runs, reducing noisy local failures while preserving stricter branch validation when maintainers need it.
  - **[Internal]** - Added a committed-review regression case for change-focused review-title rendering, so the adjudicated corpus now protects the final header from collapsing to an uninformative bare PR number when the review already established a richer change summary.
  - **[Internal]** - Extended moderator-owned presentation hints with compact titled-finding `impact` and `evidence` fields, and updated the presentation regression case so the corpus now protects the new issue readability layout without letting the render layer invent user-facing finding summaries.
  - **[Internal]** - Extended moderator-owned presentation hints and the presentation payload schema so surviving non-empty issues, observations, and recommendation findings must arrive with structured summary, impact, and evidence fields, and aligned the changelog taxonomy validator with the documented single blank-line separator between `Unreleased` groups.
  - **[Internal]** - Added the `review-coordinator` runtime skill, expanded the shared review contract with `REVIEW-COORD-*` deterministic coverage-routing rules, and added adjudicated regression coverage for overlap-surface inspection on new resources so reruns over the same change-set hold a stable review surface order.
  - **[Internal]** - Added the shipped `review-coverage-matrix.schema.json` runtime artifact and tightened the review prompts and router skill around explicit overlap rows and pre-routed-role completion checks, so the coverage gate is now mechanically representable instead of remaining prose-only.
  - **[Internal]** - Tightened the local and committed review prompts plus the router skill around matrix-build versus matrix-validation sequencing, and added adjudicated regression coverage for that ordering rule so standards loading no longer conflicts with deterministic coverage completion.
  - **[Internal]** - Tightened structured completion semantics for the coverage matrix and added regression coverage for explicit not-applicable issue-class handling, so row and matrix completeness now have more symmetrical machine-readable state.
  - **[Internal]** - Tightened the router skill and prompt Step 3A wording around an explicit validation sub-phase, and added adjudicated regression coverage for that canonical completion gate so future hardening can stay inside the router until a separate validator surface is actually justified.
  - **[Internal]** - Added generic synthetic committed-review regression cases for overlap-lifecycle ownership gaps, unsupported diagnostic-status failure concerns, and conditional required-field docs parity, so these benchmarks no longer depend on a live service surface or current repo file layout.
  - **[Internal]** - Regression cases now embed maintainer provenance directly in the canonical case schema and HCL authoring surface, and the artifact validator now enforces synthetic-only runnable corpus content outside those provenance fields so historical origin can be preserved without reintroducing repo-shape drift.
  - **[Internal]** - The regression harness now generates ignored provenance reports from embedded case metadata under `tools/regression/results/latest/`, and the artifact validator now rejects historical references and PR-number leaks outside provenance fields so runnable fixtures, examples, and case bodies stay fully synthetic.
