...
- Refactored `.github/prompts/code-review-local-changes.prompt.md` and `.github/prompts/code-review-committed-changes.prompt.md` to consume a shared code review compliance contract, keep the pirate no-changes hard-stop messages, and require a dedicated `azurerm-linter` section with explicit `Issues found` / `No issues` / `Not applicable` / `Not run` status reporting.
...
- Removed committed-review prompt startup commands that no longer feed the review output or standalone `azurerm-linter` flow (`git log --oneline origin/main..HEAD` and `git status --short`), while keeping branch and diff commands that still drive scope, summary, and reporting.
- Clarified `azurerm-linter` handling in both generic review prompts: a local install is preferred but not mandatory, `go run github.com/qixialu/azurerm-linter@latest` remains an acceptable fallback, and the linter section now includes an install hint with the upstream repo and `go install` command when the binary is missing or the tool cannot be run for availability reasons.
- Updated the shared review contract and both generic review prompts so `azurerm-linter` findings are always reported as issues for the executed linter scope, with scope mismatches disclosed separately instead of downgrading findings based on contributor guidance.
- Tightened the generic review prompts and shared linter rules to match real `azurerm-linter` output more closely: the linter section now captures issue count from the `Found X issue(s)` footer, separates execution logs from actual violation lines, and prefers repo-relative path normalization for worktree-based output when deterministic.
- Added explicit handling for the `azurerm-linter` no-work case: when the tool reports zero changed files or otherwise prints `Error: no packages to analyze`, the generic review prompts now treat that result as `Not applicable` instead of `Not run`.
- Added explicit handling for `azurerm-linter` flag and usage parse errors so generic review prompts classify them as `Not run` due to invocation error, avoid showing install hints unless the binary is actually missing, and keep findings empty for that case.
- Fixed generic review prompt guidance for `azurerm-linter` execution by requiring the tool to run from the resolved git repo root and by recording the working directory in the linter section, which avoids subdirectory execution drift when reviewing provider changes.
- Clarified both generic review prompts so their required git and `azurerm-linter` commands are already authorized by the prompt, avoiding extra user confirmation requests for `git rev-parse --show-toplevel` and normal linter execution.
- Clarified both generic review prompts so `git rev-parse --show-toplevel` is an automatic linter-step command used to resolve the repo root, rather than something the review should ask the user for in chat.
- Tightened both generic review prompts again so mandatory git and linter commands must be executed immediately when their step applies, without a confirmation turn or approval-seeking preamble.
- Tightened both generic review prompts and the shared linter contract so `azurerm-linter` gets a longer sync timeout and must be allowed to finish before the review classifies the linter section, avoiding premature `Not run` results from partial output.
- Documented the narrow VS Code user-setting override for `git rev-parse --show-toplevel`, so users can auto-approve the harmless repo-root lookup command without broadly approving other terminal commands.
- Changed generic review prompt integration for `azurerm-linter` to use filtered mode first and only run a narrow package-scoped `--no-filter` fallback when deletion-only diffs or `0 changed lines` indicate filtered results may be incomplete; the linter section now reports primary filtered runs and fallback package runs separately.
- Tightened generic review prompt handling of azurerm-linter output by requiring each lint run to be captured to a saved log file, basing summaries on the full saved output instead of terminal snapshots, and prioritizing reviewed-file findings from fallback `--no-filter` runs before summarizing broader package-scope pre-existing issues.
- Narrowed generic review prompt reporting for fallback `--no-filter` runs so only reviewed-file findings are enumerated by default, while unrelated broader package-scope findings are summarized as omitted noise instead of being surfaced as review issues.
- Required `azurerm-linter` findings to appear in the main review `ISSUES` section in addition to the linter execution subsection, so fallback `--no-filter` violations are not lost in the execution report.
...
- Simplified the new `azurerm-linter` review integration to assume the fixed filtered-mode behavior as the baseline: local and committed review prompts now run a single filtered pass, report one linter execution block without fallback fields, and stop carrying deletion-only `--no-filter` workaround flow in ordinary reviews.
- Simplified the new `azurerm-linter` review presentation so successful runs emphasize concise status, scope, summary, and findings, while raw execution details such as command, working directory, log file path, and install hints are only shown when relevant for failures or debugging.
- Tightened the new `azurerm-linter` review baseline to require a locally installed binary: generic review prompts no longer fall back to `go run` from the remote module path and instead report `Not run` with an install hint when the tool is not present locally.
- Reduced the generic review prompt `azurerm-linter` subsection to five required reviewer-facing fields only (`Status`, `Run Scope`, `Issue Count`, `Summary`, and `Must Fix`) and renamed inline linter violations from `Findings` to `Must Fix`.
- Tightened `Must Fix` formatting in the generic review prompts and shared contract so zero linter violations must render as `None`, while multiple violations must render as newline-separated `CHECKID path:line: message` entries instead of a concatenated sentence.
- Simplified generic review prompt linter execution so normal reviews use a single direct `azurerm-linter` invocation from the repo root without generated helper scripts, persisted temp log files, or extra execution scaffolding when output parsing is ambiguous.
- Differentiated generic review prompt linter scope by review type: local review now stays on a direct filtered local-diff invocation, while committed review requires a deterministically discovered PR number and uses `azurerm-linter --pr=<number>` or reports `Not run` with draft-PR guidance when no valid PR can be determined.
