...
- **User-Priority:**
  ...
  - **[Review]** - Reviews of variant-specific or overlap-prone resources now check ownership boundaries and destructive lifecycle paths more consistently, improving the chance that “wrong resource can be managed or deleted” defects are surfaced as blocking issues.
  - **[Review]** - Review output is now more consistent across reruns, with a shared rendered format, clearer finding titles, stable stage execution, and deterministic final moderation even when no issues survive.
  - **[Review]** - Review runs now fail earlier and more clearly when required workflow assets, PR file scope, or final presentation data are missing, reducing confusing partial-review outcomes.
  - **[Review]** - Docs examples are now checked more directly against tested or implementation-backed Terraform shapes, making it less likely that published examples drift from what the provider actually supports.
  - **[Review]** - New-resource reviews now hold more strongly to the expected acceptance-test baseline, including preserving missing broader `complete` coverage as a distinct concern instead of letting it disappear behind unrelated findings.
