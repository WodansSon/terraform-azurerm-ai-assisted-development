...
> [!NOTE]
> The generic review prompts require a local `v0.2.0` or newer `azurerm-linter` binary from the [QixiaLu/azurerm-linter](https://github.com/QixiaLu/azurerm-linter) repo for JSON-mode review. The contract-backed review flow runs the local CLI from the repo root and classifies the result deterministically. For runtime details and known failure modes, see [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md).
