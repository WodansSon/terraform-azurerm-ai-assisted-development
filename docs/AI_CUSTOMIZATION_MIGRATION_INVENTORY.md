...
| `.github/prompts/code-review-committed-changes.prompt.md` | Explicit user-facing entrypoint for committed or PR-scoped review with deterministic output and authoritative-scope handling | Yes | Shared review logic and common report-generation behavior can gradually move behind the prompt; committed-scope resolution stays compatibility-sensitive | High risk: prompt owns PR-scope resolution behavior and exact failure text | Keep prompt stable; extract only clearly shared logic after regression coverage is expanded |
