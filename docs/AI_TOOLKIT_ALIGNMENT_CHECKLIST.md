...
This checklist is for maintainers of the Interactive Toolkit in this repository. Hosted Toolkit architecture and validation are owned by `docs/HOSTED_COPILOT_CODE_REVIEW_ARCHITECTURE.md` and `hosted_copilot/tools/Test-Toolkit.ps1`.
...
In the default text mode, the validator starts with a blank line and a divider-framed `AI TOOLKIT VALIDATION` heading so its output is visually separate from the invoking shell prompt. It then reports each step in aligned status, name, and detail columns using `[RUNNING]`, `[PASSED]`, `[FAILED]`, or `[SKIPPED]` so long-running checks remain visible. While the regression harness or upstream drift checker runs, the validator also relays its aligned tagged substep statuses and continues buffering the complete child output for summary parsing and failure details. After execution, divider-framed uppercase headings separate the overall summary, a compact step/status/duration table, highlights, and failures; routine step descriptions stay out of the passing table and remain available in JSON or alongside failures. All human-readable execution status values are uppercase, while internal objects and JSON retain their existing lowercase status values. The report ends with a blank line so the next shell prompt is visually separate. JSON mode suppresses all progress and presentation output and returns only the machine-readable summary beginning directly with `{`.
...
```powershell
pwsh -NoProfile -File ./tools/Validate-InteractiveToolkit.ps1 -FixNpmAudit
```
