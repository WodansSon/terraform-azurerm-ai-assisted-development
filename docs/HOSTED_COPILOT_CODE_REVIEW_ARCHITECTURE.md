...
`Start-RuleWorkbench.ps1` normally refreshes static UI assets and serves the last validated display from `%LOCALAPPDATA%\hosted-workbench\site` at a stable loopback origin. It performs no source collection, assessment, or reconciliation unless the maintainer explicitly supplies `-Rebuild`. The target cold-start design adds a catalog-owned portable assessment bootstrap under `hosted_copilot/copilot-rule-catalog/rule-assessments/`. During an explicit rebuild, the launcher resolves each source through machine-local context cache, the validated bootstrap, and finally the local Copilot CLI. Until that bootstrap is implemented and published, a new maintainer machine must evaluate the complete assessment set once before local cache reuse becomes effective.

The local assessment cache context includes the source record, Hosted catalog, evaluator contract, prompt, schema, model, and reasoning effort. The local reconciliation cache binds each source-defined assessment batch to the Hosted catalog, reconciliation contract, prompt, schema, evaluator, model, and reasoning effort. Cache hits are schema-validated and passed through the trusted display builder before reuse; changed identities reevaluate only their affected records or batches. Each model batch runs from an isolated temporary directory containing only its bounded inputs; Copilot receives only the read-only `view` tool, returns structured JSONL, and must report the configured model. The command retries malformed output within a fixed bound, writes cache and bundle artifacts outside the repository, and never applies catalog or ledger changes. Supplying `BundlePath` directly to the assessor bypasses collection but still resolves assessments.

`workbench-display.json` is the durable last validated presentation snapshot, not semantic cache state or a freshness authority. Ordinary launch validates and serves it without semantic work. Explicit `-Rebuild` resolves current assessment and reconciliation results through their independent caches and recovery paths, builds a candidate display in an isolated run directory, schema-validates it, and atomically replaces the durable snapshot only after success. A failed rebuild leaves the last good display untouched and retains reusable semantic work for the next explicit rebuild.

#### Ownership And Consolidation:

Canonical catalog mappings exclusively own existing Hosted rule identity. Related coverage, semantic similarity, and consolidation analysis are advisory evidence and must never infer or override ownership.

- Each ordinary candidate retains its source-bound assessment identity and one catalog-owned target when mapped.
- A final reconciliation result that assigns multiple ordinary recommendations to one existing Hosted target is invalid output. Retry it within the bounded evaluator policy, then fail before Workbench launch when it remains invalid.
- Do not convert duplicate target ownership into a blocked display, conflict tab, MessageBar, or maintainer conflict-adjudication workflow. Maintainers should never repair evaluator ownership inside the browser.
- Preserve semantic overlap separately as consolidation evidence. A future consolidation proposal may group related assessments and existing rules, recommend one new collision-free Hosted ID and merged wording, identify rules proposed for retirement, prove member-meaning coverage, and report before-and-after tokens.
- Keep a consolidation proposal separate from ordinary candidate decisions until a maintainer explicitly accepts it, splits it, edits it, or keeps the existing rules unchanged. Acceptance adds the new rule and retires only the selected existing rules as one atomic proposal; it does not overwrite an existing rule ID.

This is maintainer-invoked experiment support, not unattended semantic synchronization. A changed Interactive rule or upstream document invalidates only its matching bootstrap and cache entries when the remaining context is unchanged. A changed Hosted catalog invalidates bootstrap entries whose equivalence judgments used that catalog; evaluator-contract, prompt, schema, model, or reasoning changes invalidate entries whose recorded context no longer matches. The future bootstrap publisher previews by default and updates shared assessment state only with explicit publish approval after a maintainer accepts a complete assessed bundle. Required CI validates published semantic output but never regenerates it unattended. The server binds only to `127.0.0.1`, accepts only matching `127.0.0.1` or `localhost` Host values to prevent DNS rebinding, serves static files through `GET` and `HEAD` under a local-only content security policy, exposes no repository-write API, and uses a stable default port so browser storage remains available across launches. **Close Workbench** sends the sole mutating request, a random per-launch-token-authenticated `POST /shutdown` that stops only the local server process. The browser receives no GitHub token, SSH private key, or other GitHub CLI credential. A port override creates a different browser-storage origin and must be reported clearly.

The Workbench uses IndexedDB for review bundles, read-only AI assessments, maintainer decisions, provisional applicability overrides, evidence notes, and resumable drafts. Local storage holds only lightweight preferences and the active session identifier. Each persisted decision, assessment, and override is keyed to source identity and content hash; changed source content reopens the candidate instead of inheriting stale analysis. Draft schema version 4 stores a mutually exclusive rule action, trusted generated Hosted identity, decision rationale, promotion-plan membership and provenance, source-bound bulk-operation records, and source-bound provisional overrides; unreleased earlier draft shapes are rejected rather than migrated. Selection payload schema version 3 carries the same membership provenance into the hash-bound approval handoff. Maintainers can export and import current draft state independently from browser storage.
...
Present the Workbench as a specialized IDE using a product-owned dark theme whose initial shell language references VS Code Dark 2026: a compact title bar with the yellow JSON braces Codicon as the product mark, icon-only activity navigation, editor-style outer tabs, panel-style inner tabs, charcoal editor surfaces, and a compact bottom status bar. Scope its tokens under `theme-hosted-dark`; do not import or depend on an installed VS Code theme at runtime. Keep independently pinned, locally vendored icon families under `workbench/icons/codicons/` and `workbench/icons/octicons/`, each with its own generated SVG sprite, license, attribution, and offline-only generator. Use Visual Studio Code Codicons for the IDE shell and general Workbench controls. Use GitHub Primer Octicons only for GitHub-familiar review controls inside Preview. Keep the Workbench free of runtime icon-CDN or package dependencies. The title bar owns Draft options and the far-right Close Workbench action. The activity rail owns stage navigation plus one lower-right Promotion Plan count badge that hides at zero, shows exact counts through `999`, and displays `999+` above that threshold. The badge expands left from a fixed right edge and preserves the exact localized count in the stage tooltip and accessible label. The status bar identifies the authenticated user's validated writable AzureRM fork, default branch, and pinned target commit, and remains the exact-count authority for autosave state, eligible and excluded counts, mapping counts, full promotion-plan count, test-guidance headroom, and preview readiness. Fork synchronization is an informational housekeeping signal only: keep the target neutral when synchronized or ahead-only, use amber when its branch is behind the HashiCorp parent branch, list only nonzero `↓ N` and `↑ N` counts after pipe separators in the target tooltip, and never block Workbench behavior on sync state. Status labels stay icon-first with accessible names and tooltips, use the standard pointer, and truncate without replacing a parent-owned tooltip.
...
Large Promotion Previews retain every file header and exact body height for continuous navigation, but keep diff-row DOM mounted only for files within two Preview viewports. File navigation hydrates its target before interaction, and browser or tree-pane width changes remeasure cached heights without changing scroll geometry.
...
Both outer tab children organize results beneath non-selectable **Interactive Toolkit**, **Contributor Guidance**, and **Maintainer Proposals** source roots. Switching outer tabs changes visibility only and preserves each tab's folders, selection, sort state, scroll position, rendered detail, and independent search query. Outer and nested tabs use uppercase text without decorative icons, isolated tab surfaces on the editor background, and a bottom-edge active indicator. Column headers belong to each row-bearing subsection rather than floating globally above unrelated folders. Candidate Sources and Assessment Results share one sortable-header treatment, sort only their owning subsection, and default to Candidate ascending with a visible direction chevron immediately beside the active label. Candidate labels are left-aligned, remaining label-and-chevron groups are centered, and wide screens cap the Candidate track so status columns remain visually associated with their rows. Data headers use product theme surfaces rather than legacy view-specific colors. Source and category counts use labeled blue count badges with white semibold Title Case text, while other status capsules retain their semantic colors and shared Title Case treatment. Visually ellipsized text exposes its complete text through a generated tooltip. Interactive and maintainer rules retain navigation-only category folders in the candidate view, while Contributor Guidance rules are direct children of their source root. Each eligible candidate shows source lifecycle and authoritative Hosted catalog status separately. `Mapped` means the bundle identifies one or more active Hosted rules; `Retired mapping` means only retired Hosted rules remain; `Not mapped` means no authoritative mapping exists. In Tokens, unsigned values show current guarded usage and signed values show an explicit action's estimated delta. When an overridden exclusion has no AI-generated token delta, estimate its maintained proposed text with the same guarded character-quarter method for Candidate, Details, Plan, and capacity displays. The eligible-tree checkbox controls only promotion-plan membership and never infers an action. Clicking or keyboard-activating a candidate row opens its complete source rule and AI assessment in the full-width Details view without changing plan membership or collapsing the tree. When a candidate or assessment-result row has focus, Up and Down Arrow move selection and focus to the previous or next visible row, clamp at list boundaries, and scroll the target into view. Folder summaries retain native disclosure-key behavior, and focused promotion-plan checkboxes retain native checkbox-key behavior.

The **Contributor Guidance** source root in both outer tab children shows a non-interactive green provenance pill containing the HashiCorp contributor-reference repository, ref, and short immutable commit from the intake bundle. Place it `8px` after the source label, preserve one aligned trailing count track at supported compact widths, and expose the full commit in its tooltip. Other source roots do not show upstream provenance.
...
- **Catalog:** Search evaluated upstream and Interactive candidates, compare source lifecycle with Hosted catalog status, add individual candidates to the promotion plan with leaf checkboxes, or use the explicit **Bulk Actions** menu to add currently visible candidates whose AI recommendation is Add or Update. Bulk selection changes membership only and never infers a Rule Action or rationale. Every bulk operation records only newly added candidate keys, its current-results query, recommendation scope, operation ID, and timestamp.
- **Assessment:** Display the complete source rule, exact mapped Hosted rule IDs, text and placement, separate related-rule analysis, AI recommendation, impact description, token cost, projected headroom, all factor ratings, selection rationale, and proposed wording. Show the trusted generated identity as an immutable `RULE: <HOSTED_ID>` header. The maintainer stages one status-constrained action, records rationale, and saves both as one decision. Saving Add, Update, Retire, or Restore enters the candidate into the promotion plan; choosing `No Change` clears the saved decision, disables rationale, and preserves the Details scroll position and selected-radio focus. AI recommendation remains advisory.
- **Promotion plan:** Collect saved promotion actions, explicit eligible-tree selections, applicability overrides, and bulk-added candidates while preserving their membership provenance. Require an explicit Add, Update, Retire, or Restore plus rationale before approval. Editing a bulk-added candidate individually promotes its membership to manual ownership. Bulk Undo removes only candidates still owned by the matching operation and never removes pre-existing or subsequently reviewed manual selections. Per-row Undo removes the complete saved decision and any provisional override so action, rationale, proposed text, membership, list placement, and selection return to defaults; immediate Restore reinstates the full decision, provenance, operation membership, and override snapshots from the notification.
...
- **Preview:** Render every planned add, update, and retirement as a GitHub-style unified rule diff with old and new line-number gutters, dedicated `+` and `-` marker columns, exact green additions, exact red removals, neutral context, and action context. Compare only changed selection-payload fragments side by side against default Workbench selection state, using one line-number gutter and one body-colored marker column per side. Retain the complete raw selection JSON in a collapsed audit disclosure with compact gray chevrons. Highlight and navigate complete in-plan Add, Update, and applicability-override records in the raw payload; when manual scrolling moves the active change entirely outside the viewport, enable the directional control that returns to it. Do not offer or retain an `update` action when current and proposed Hosted rule text are identical.
- **Approval:** On Preview, require selected-rule rationale and manual approver identity before **Approve & Export** freezes the exact selection payload, including membership provenance and bulk-operation records, calculates its SHA-256 hash, and exports an immutable approval handoff. Any later selection, provenance, or rationale change requires a new handoff.
...
Experiment cleanup is also a mutation boundary. Derive disposable head branches from validated `source-pr-*` case and run identifiers, require exact `control-review/` and `hosted-review/` namespaces, and validate Git refs before closing pull requests or deleting local or remote branches.
