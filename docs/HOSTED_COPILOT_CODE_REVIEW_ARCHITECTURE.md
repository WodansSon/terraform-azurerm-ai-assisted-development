...
`Start-RuleWorkbench.ps1` invokes the deterministic collector and incremental semantic assessor, stages the static Workbench with the completed bundle in an external temporary directory, and serves it from a stable loopback origin. The target cold-start design adds a catalog-owned portable assessment bootstrap under `hosted_copilot/copilot-rule-catalog/rule-assessments/`. The launcher resolves each source through machine-local context cache, the validated bootstrap, and finally the local Copilot CLI. Until that bootstrap is implemented and published, a new maintainer machine must evaluate the complete assessment set once before local cache reuse becomes effective.
...
`workbench-display.json` is disposable presentation output, not cache state or a freshness authority. Every normal launch removes the prior staged display, resolves current assessment and reconciliation results through their independent caches and recovery paths, builds a new display in the launch run directory, and atomically publishes it only after successful validation. A display-write failure therefore retains reusable semantic work without making the next launch rerun unchanged model evaluation.
