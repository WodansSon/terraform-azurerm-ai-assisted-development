...
Present the Workbench as a specialized IDE using a product-owned dark theme whose initial shell language references VS Code Dark 2026: a compact title bar with the yellow JSON braces Codicon as the product mark, icon-only activity navigation, editor-style outer tabs, panel-style inner tabs, charcoal editor surfaces, and a compact bottom status bar. Scope its tokens under `theme-hosted-dark`; do not import or depend on an installed VS Code theme at runtime. Use the pinned, locally vendored Visual Studio Code Codicon source set and generated SVG sprite for interface icons and the title mark, and keep the Workbench free of runtime icon-CDN or package dependencies. Preserve the Codicons license and attribution beside the vendored SVG sources. Keep Workbench-specific semantic colors for recommendations, risk, readiness, and diffs. The title bar owns Draft options and the far-right Close Workbench action. The activity rail owns stage navigation plus one lower-right Promotion Plan count badge that hides at zero, shows exact counts through `999`, and displays `999+` above that threshold. The badge expands left from a fixed right edge and preserves the exact localized count in the stage tooltip and accessible label. The status bar remains the exact-count authority for source snapshot, autosave state, eligible and excluded counts, mapping counts, full promotion-plan count, test-guidance headroom, and preview readiness; labels stay icon-first with accessible names and tooltips.
