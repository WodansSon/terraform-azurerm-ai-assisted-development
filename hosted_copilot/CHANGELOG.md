...
- Improved Workbench usability across supported viewports with consistent Hosted dark-theme presentation, independently owned VS Code Codicon and GitHub Primer Octicon families with visible PNG inventories, accessible semantic typography, persistent rule-identity headers, breakpoint-verified local scrolling, membership-only plan toggles that preserve Details and tree context, shared sortable headers, an explicit writable-fork promotion target with non-blocking sync housekeeping, aligned green Contributor Guidance provenance pills, neutral status treatments, canvas-contained status tooltips, a compact Promotion Plan activity badge with exact footer counts, GitHub-familiar diff gutters and scroll-aware raw-change navigation, a compact icon-only rationale save action, a self-contained branded shutdown state, complete truncation tooltips, non-duplicated status information, and reliable Candidate, Details, Assessment, Plan, and Preview workflows.
