...
- Added thirty-eight Interactive Toolkit rules that the HashiCorp contributor guide does not already mandate, covering nearby-implementation pattern reuse, ephemeral-resource and provider-defined-function surfaces, inline validator composition, comment restraint, redundant lifecycle logging, one-use configuration-helper inlining, non-resource documentation types, example-rewrite fidelity, schema shape parity, note hygiene, and documentation wording, and grouped them under new `Specialized Surfaces`, `Code Clarity`, `Schema Shape Parity`, and `Notes` sections.
