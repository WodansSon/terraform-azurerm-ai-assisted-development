...
- Improved Workbench usability across supported viewports with shared model-driven Candidate Sources and Assessment Results trees, complete sticky parent rows, sort-stable scrolling, jump-free folder expansion, generated Hosted identities and proposals throughout Assessment Results, compact icon-only override actions, semantically matched scoring meters and values, stable row and scrollbar alignment, accessible controls, hierarchical readiness cues, non-destructive candidate search, consistent clamped tooltips and tree truncation, unobtrusive VS Code-style scrollbars, reliable Bulk Actions, clearer status feedback, and consistent Candidate, Details, Assessment, Plan, and Preview workflows.
