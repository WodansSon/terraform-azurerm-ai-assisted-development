...
- Made assessment identity source-local while reconciliation owns complete, collision-free Hosted rule recommendations, including contingent metadata for deferred and excluded assessments and explicit implementation-model scope for implementation additions.
...
- Made Workbench startup report assessment and reconciliation progress, reuse unchanged assessments, automatically resume compatible failed runs, evaluate source-defined reconciliation batches with bounded parallelism, preserve deterministic merging and exhaustive validation, and keep diagnostic cache, recovery, and concurrency overrides available without burdening the normal launch command.
- Preserved every failed reconciliation attempt with its validation reason and changed duplicate-target final merges from fatal startup failures into schema-valid blocked Workbench displays that retain conflict evidence, keep unaffected candidates reviewable, and disable approval until conflicts are resolved.
