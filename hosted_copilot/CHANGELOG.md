...
- Simplified Workbench Rule Actions around trusted generated identities and one saved action-and-rationale decision: the Details pane now shows an immutable rule header, promotion actions imply plan membership, and `No Change` clears the decision instead of requiring a second inclusion control.
- Made ordinary Workbench startup a display-only operation that refreshes UI assets and serves the durable last validated snapshot; semantic collection, assessment, and reconciliation now require explicit `-Rebuild`, which preserves the last good display until a replacement passes validation.
