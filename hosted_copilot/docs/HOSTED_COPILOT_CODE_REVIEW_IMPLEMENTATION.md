...
Render status help through one fixed Workbench-owned tooltip rather than native `title` attributes. Read each status item's current `data-status-tooltip` value when it is hovered or focused, reset the tooltip measurement origin before calculating placement, use intrinsic `max-content` width capped to the viewport, position it `6px` above the status bar, and clamp both horizontal edges inside the rendered canvas. Hide it on pointer exit, focus exit, and resize.
...
Every candidate has one explicit action. A mapped candidate exposes `update` only when its proposed Hosted text differs from the current active mapped text; identical text is a no-op, so stale updates normalize to `no-change` and leave the plan. Bulk operations persist an operation ID, timestamp, current-results query, recommendation scope, and only the candidate keys newly added by that operation. Existing manual and override selections remain untouched. Individually editing a bulk-added candidate promotes its membership provenance to manual; bulk Undo removes only candidates still owned by the matching operation. The current Preview action remains disabled until at least one add, update, or retire action is in the plan, every plan item has maintainer rationale, and an approver name is present. **Approve & Export** writes a hash-bound selection handoff containing catalog status, mapped Hosted IDs, action, plan membership provenance, and bulk-operation records; it does not update the repository. Undo or any later action, membership, provenance, or rationale edit requires exporting a new handoff with a different payload hash.
