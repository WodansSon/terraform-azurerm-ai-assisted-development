...
##### Next Implementation Step: Reconciliation Conflict Adjudication

Preserve failed evaluator drafts and distinguish correctable evidence errors from semantic conflicts that require maintainer judgment.
...
- Let an authenticated maintainer merge recommendations, keep one and defer or exclude another, or split a distinct rule into a new tentative ID.
- Disable promotion approval and catalog mutation while the display contains unresolved conflicts; enable them only after every conflict has an explicit maintainer resolution and the resolved full-corpus draft passes the trusted builder.
...
`Test-RuleWorkbenchHeaded.ps1` provides visible playback through the same Playwright runner. By default it owns a temporary Workbench server and staging directory, fails before launch when its selected port is occupied, clicks **Close Workbench** after the final journey, verifies the closed screen and stopped server, and removes all owned staging in a `finally` path. Explicit `-Url` usage is attach-only and must never stop or remove an externally owned Workbench.
