...
Use the complete available width for each Candidate Sources replacement view; do not compress Candidates and Details beside each other or introduce a breakpoint-specific split. Style outer workspace tabs as editor tabs and nested pane tabs as compact panel tabs without changing their domain-specific labels or behavior. Candidate Sources and Assessment Results share one sortable-header treatment: Candidate is left-aligned, remaining columns are centered, and every active chevron occupies a reserved slot that cannot shift or overlap its label. All data headers use the product theme surface and line tokens. Status capsules share one height, padding, border radius, type treatment, and Title Case labels while retaining semantic colors. Neutral summary and hierarchy counts use the dark surface fill, lighter structural border, and muted text from `theme-hosted-dark`. Section titles use one large white Title Case style with a trailing colon; subordinate labels use one smaller white Title Case style with a trailing colon. Summary headings place the title on the left and status capsule on the right. Any text visually truncated with an ellipsis exposes its complete current text through a generated native tooltip without adding a static-content tab stop.
