...
Promotion Preview uses a GitHub-style changed-file review shell. Keep a filterable changed-file navigator synchronized with Proposed Changes, Payload Changes, and Raw Selection Payload; each scope owns its file count, diff totals, visible files, and jump targets. Render proposed rules and changed selection records as split files with Current file on the left and Changes on the right, compact file headers, diffstat, copy-path, collapse, and Viewed controls. Keep the existing approval and export workflow behind Review changes rather than in a persistent sidebar. At 900px and below, stack the changed-file navigator above the contained review surface.

Render Raw Selection Payload as one complete `selection/promotion-selection.json` file rather than one file or disclosure per decision. Preserve each changed row in its true file position, show exactly two unchanged context lines before and after changes, and collapse larger unchanged ranges into blue hunk rows. Use the pinned local `fold-up-16`, `unfold-16`, and `fold-down-16` Octicons in a `48px` control matching the line-number gutter. Fold Up and Fold Down reveal ten lines per activation from the nearest hunk edge; Unfold reveals the complete range between hunks.
...
- `WB-UX-PREVIEW-001`: Promotion Preview synchronizes changed-file navigation across Proposed, Payload, and Raw review scopes.
- `WB-UX-PREVIEW-002`: Proposed and Payload scopes use contained split-file review surfaces.
- `WB-UX-PREVIEW-003`: Raw Selection Payload remains one complete contextual file diff.
- `WB-UX-PREVIEW-004`: Raw context controls use local directional Octicons with exact expansion behavior.
- `WB-UX-PREVIEW-005`: Review approval remains available through Review changes across supported layouts.
