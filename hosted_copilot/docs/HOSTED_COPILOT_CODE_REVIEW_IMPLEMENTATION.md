...
An authenticated Hosted CODEOWNER can add a source-bound provisional override to an AI-excluded result with a required rationale. Applying the override removes the row from active Assessment Results, adds it to Overrides and the promotion plan with `No Change` still selected, and retains the original AI exclusion plus override record in persisted audit data. Explicit Undo and Remove Override atomically remove both the decision and provisional override, return the candidate to active Assessment Results, and clear stale selections; Restore reinstates both snapshots. Carry the audit record and plan-membership provenance through draft schema version 5 and selection payload version 4. Treat this as trusted maintainer discretion; do not describe it as unbypassable multi-party enforcement.
...
Every rule candidate has one explicit action. Provenance links identify evidence sources and never become action targets. An update or retirement identifies exactly one active Hosted rule; an addition has no existing target. A mapped candidate exposes `update` only when its complete proposed Hosted rule differs from that target's current text; identical text is a no-op, so stale updates normalize to `no-change` and leave the plan. Bulk operations persist an operation ID, timestamp, current-results query, recommendation scope, and only the candidate keys newly added by that operation. Existing manual and override selections remain untouched. Individually editing a bulk-added candidate promotes its membership provenance to manual; bulk Undo removes only candidates still owned by the matching operation. The current Preview action remains disabled until at least one add, update, or retire action is in the plan, every plan item has maintainer rationale, and an approver name is present. **Approve & Export** writes a hash-bound selection handoff containing source identity, rule-candidate identity, singular Hosted target, action, plan membership provenance, and bulk-operation records; it does not update the repository. Undo or any later action, membership, provenance, or rationale edit requires exporting a new handoff with a different payload hash.
