...
Treat authoritative catalog status as persistent rule identity in every Candidate and Assessment Details state, independently from source lifecycle, AI recommendation, or maintainer action. Render `Mapped`, `Not Mapped`, or `Retired` as the final capsule in the fixed Details header. Do not repeat catalog status, mapping counts, or unmapped explanatory copy in the scrolling body. When mapped Hosted rules exist, render only their substantive cards under `MAPPED HOSTED RULES:`; omit that section when no mapped rules exist.
