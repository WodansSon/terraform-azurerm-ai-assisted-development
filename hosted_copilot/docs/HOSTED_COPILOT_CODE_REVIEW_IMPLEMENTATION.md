...
Every assessment carries a complete proposed Hosted rule independently from its recommendation: `proposedHostedRuleId`, proposed text, category, and affected surfaces. `targetHostedRuleId` separately identifies the existing catalog rule when one exists. Show the proposed Hosted rule ID inside the grouped Rule Actions controls for every candidate. Keep it editable when no target exists; for mapped updates, retirements, deferrals, and no-change proposals, display the generated ID read-only and equal to the existing target because the promotion contract does not define rename semantics. Limit editable IDs to 32 characters and require the catalog rule-ID format. Reject active or retired catalog collisions except for the candidate's exact target, and reject IDs reserved by any other proposed assessment or maintainer decision. Persist the value in resumable drafts and selection exports, and use it as the resulting candidate identity in the tree, fixed Details header, Promotion Plan, Preview, and search without changing the source-bound candidate key. Keep invalid editable text available, show a red compact circle-slash and the specific reason, and block promotion readiness until corrected. Show a green compact check and **Proposed Hosted Rule ID is valid.** when valid. Put that status and the live character count on one metadata row below the input, update the count immediately, debounce validation for 300 milliseconds, validate immediately on blur, and do not rebuild unchanged status content while the maintainer types.
...
In Preview, render Proposed Changes as a GitHub-style unified diff with old and new line-number gutters followed by a body-colored marker column. Render the side-by-side selection comparison with one tinted line-number gutter and one body-colored marker column on each side. Reuse the exact rendered green and red row colors, number colors, white code text, and gray structural dividers across Proposed Changes, Payload Changes, and Raw Selection Payload. Raw Selection Payload uses compact muted chevrons for its disclosure and separately highlights and navigates complete in-plan Add, Update, and applicability-override decision records. If manual scrolling moves the active change block entirely above or below the raw viewport, enable the corresponding navigation chevron to snap back to that same block even when it is the only change. Disabled raw-navigation chevrons retain the normal arrow cursor; enabled chevrons use the pointer cursor. An unresolved override remains visible in this audit even though approval stays blocked until the maintainer chooses an explicit promotion action.
...
- `WB-UX-PREVIEW-004`: Raw Selection Payload remains one complete contextual file diff with exact expansion behavior.
...
Before installing or executing Node validation dependencies, the complete Workbench suite must run `npm audit` against the complete lockfile with package scripts disabled and fail immediately on high or critical findings. Default validation must not modify either dependency manifest. `-FixNpmAudit` may stage ordinary npm fixes and same-major transitive overrides in a temporary directory, re-audit the complete staged graph, and apply both manifests only after the staged graph is clean. Require the additional `-AllowBreakingNpmFix` switch before evaluating npm's breaking direct-dependency recommendation, and still reject it when the resulting graph does not pass the same audit. After applying a clean remediation, install the resulting lock and run the complete Workbench suite.
