...
"@, [Text.UTF8Encoding]::new($false))
    ...
    $octiconContractValid = @(Get-ChildItem -LiteralPath $iconRoot -File).Count -eq 0 -and $octiconSourceCount -eq 38 -and ([regex]::Matches($octiconSpriteContent, '<symbol id="octicon-').Count -eq 38) -and $octiconSpriteContent -match 'id="octicon-code-review-16"' -and $octiconSpriteContent -match 'id="octicon-file-diff-16"' -and $octiconSpriteContent -match 'id="octicon-diff-added-16"' -and $octiconSpriteContent -match 'id="octicon-diff-removed-16"' -and $octiconSpriteContent -match 'id="octicon-fold-16"' -and $octiconSpriteContent -match 'id="octicon-fold-up-16"' -and $octiconSpriteContent -match 'id="octicon-fold-down-16"' -and $octiconSpriteContent -match 'id="octicon-unfold-16"' -and $octiconSpriteContent -match 'id="octicon-comment-ai-16"' -and $octiconSpriteContent -match '6220ff87f3ddd923b05ffdac7e2d9cb714213205' -and $octiconAttributionContent -match 'GitHub''s Primer Octicons' -and $octiconAttributionContent -match 'MIT License' -and (Get-Content -LiteralPath (Join-Path $octiconRoot 'LICENSE.txt') -Raw) -match 'MIT License' -and $octiconGeneratorContent -match '\.\.\\workbench\\icons\\octicons' -and $octiconGeneratorContent -match 'Get-Content -LiteralPath \$sourcePath -Raw' -and $octiconGeneratorContent -notmatch 'Invoke-WebRequest|Invoke-RestMethod|https://raw' -and $appContent -match 'function octicon\(name\)' -and $appContent -match 'gap\.direction === "up" \? "fold-up" : gap\.direction === "down" \? "fold-down" : "unfold"' -and $appContent -match '\$\{octicon\(iconName\)\}' -and $stylesContent -match '\.codicon,\s*\.octicon\s*\{'
    ...
    $previewEditorValid = $indexContent -match '<h2>Promotion Preview:</h2>' -and $indexContent -match '<span>Changed files</span>' -and $indexContent -match 'id="preview-review-scope"' -and $indexContent -match 'id="preview-review-toggle"[^>]*>Review changes</button>' -and $indexContent -match 'id="preview-review-popover"[^>]*hidden' -and $stylesContent -match '#preview-view \.preview-layout\s*\{[^}]*grid-template-columns:\s*296px minmax\(0, 1fr\)' -and $stylesContent -match '#preview-view \.preview-summary\s*\{[^}]*height:\s*100%;[^}]*border-right:\s*1px solid #30363d' -and $stylesContent -match '\.preview-review-toolbar\s*\{[^}]*position:\s*sticky' -and $stylesContent -match '\.preview-review-popover\s*\{[^}]*position:\s*absolute'
    Add-TestResult -Name 'preview-editor-layout' -Passed $previewEditorValid -Detail 'Preview uses a GitHub-style changed-files navigator, sticky review toolbar, scoped file review, and Review changes approval popover.'
    ...
    $previewDiffValid = $indexContent -match 'id="preview-diff"' -and $indexContent -match 'id="preview-payload-diff"' -and $indexContent -match 'id="preview-json"' -and $appContent -match 'function buildPreviewFiles\(' -and $appContent -match 'function createPreviewFile\(' -and $appContent -match 'function renderPreviewFiles\(' -and $appContent -match 'function renderPreviewFile\(' -and $appContent -match 'data-preview-file-path=' -and $appContent -match 'preview-split-labels' -and $appContent -match 'Current file' -and $appContent -match 'Changes' -and $stylesContent -match '\.preview-split-labels,\s*\.preview-split-row\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1fr\)' -and $stylesContent -match '\.preview-diff-cell\.add\s*\{[^}]*background:\s*#0f2d20' -and $stylesContent -match '\.preview-diff-cell\.delete\s*\{[^}]*background:\s*#3c1e23'
    Add-TestResult -Name 'preview-change-review' -Passed $previewDiffValid -Detail 'Preview renders Proposed, Payload, and Raw artifacts through one GitHub-style split-file review model with synchronized changed-file navigation.'

    $rawPayloadNavigationValid = $appContent -match 'PREVIEW_CONTEXT_LINE_COUNT = 2' -and $appContent -match 'PREVIEW_DIRECTIONAL_EXPAND_COUNT = 10' -and $appContent -match 'createPreviewFile\("selection/promotion-selection\.json"' -and $appContent -match 'function createContextualPreviewRows\(' -and $appContent -match 'direction: start === 0 \? "up" : index === rows\.length \? "down" : "all"' -and $appContent -match 'const selectedRows = direction === "all"' -and $appContent -match 'gap\.direction === "up" \? "fold-up" : gap\.direction === "down" \? "fold-down" : "unfold"' -and $stylesContent -match '\.preview-context-gap\s*\{[^}]*grid-template-columns:\s*48px minmax\(0, 1fr\)' -and $stylesContent -match '\.preview-context-expand\s*\{[^}]*width:\s*48px'
