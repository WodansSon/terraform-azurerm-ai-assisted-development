...
"@, [Text.UTF8Encoding]::new($false))
    ...
    $codiconContractValid = $codiconSourceCount -eq 42 -and ([regex]::Matches($spriteContent, '<symbol id="codicon-').Count -eq 42) -and $spriteContent -match 'id="codicon-discard"' -and $spriteContent -match 'id="codicon-git-commit"' -and $spriteContent -match 'id="codicon-git-branch-compact"' -and $spriteContent -match 'id="codicon-json"' -and $spriteContent -match 'id="codicon-collection"' -and $spriteContent -match 'id="codicon-collection-small"' -and $spriteContent -match 'id="codicon-new-session"' -and $spriteContent -match 'id="codicon-open-preview"' -and $spriteContent -match 'id="codicon-check-compact"' -and $spriteContent -match 'id="codicon-circle-slash-compact"' -and $spriteContent -match 'id="codicon-debug-disconnect-compact"' -and $spriteContent -match 'id="codicon-checklist-compact"' -and $spriteContent -match 'id="codicon-shield-compact"' -and $spriteContent -match '1c47ab36a4bb845c437866405c2fa67b8ca0fe36' -and $attributionContent -match 'Creative Commons Attribution 4\.0 International' -and $iconGeneratorContent -match '\.\.\\workbench\\icons' -and $iconGeneratorContent -match 'Get-Content -LiteralPath \$sourcePath -Raw' -and $iconGeneratorContent -notmatch 'Invoke-WebRequest|Invoke-RestMethod|https://raw' -and $launcherContent -match 'workbenchIconSource' -and $launcherContent -match 'Copy-Item -LiteralPath \$workbenchIconSource -Destination \$resolvedSiteDirectory -Recurse -Force' -and $indexContent -notmatch 'cdn\.jsdelivr|data-lucide|lucide\.min\.js' -and $appContent -notmatch 'data-lucide|window\.lucide|refreshIcons' -and $appContent -match 'function icon\(name\)' -and $appContent -match 'icons/codicons\.svg#codicon-\$\{name\}' -and $indexContent -match 'icons/codicons\.svg#codicon-json' -and $indexContent -match 'icons/codicons\.svg#codicon-git-branch-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-check-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-collection-small' -and $indexContent -match 'icons/codicons\.svg#codicon-circle-slash-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-debug-disconnect-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-checklist-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-shield-compact' -and $indexContent -match 'data-view="catalog"[\s\S]*?codicon-collection' -and $indexContent -match 'data-view="plan"[\s\S]*?codicon-new-session' -and $indexContent -match 'data-view="preview"[\s\S]*?codicon-open-preview' -and $appContent -match 'icon\("discard"\)'
