...
try {
    ...
    $stagedPaths = @('index.html', 'app.js', 'styles.css', 'favicon.svg', 'icons/codicons.svg', 'icons/discard.svg', 'icons/git-commit.svg', 'icons/LICENSE.txt', 'icons/ATTRIBUTION.md', 'shutdown-config.js', 'rule-intake-review.json') | ForEach-Object { Join-Path $siteDirectory $_ }
...
"@, [Text.UTF8Encoding]::new($false))
    ...
    $spriteContent = Get-Content -LiteralPath (Join-Path $iconRoot 'codicons.svg') -Raw
    $attributionContent = Get-Content -LiteralPath (Join-Path $iconRoot 'ATTRIBUTION.md') -Raw
    ...
    $codiconSourceCount = @(Get-ChildItem -LiteralPath $iconRoot -Filter '*.svg' -File | Where-Object { $_.Name -ne 'codicons.svg' }).Count
    $codiconContractValid = $codiconSourceCount -eq 44 -and ([regex]::Matches($spriteContent, '<symbol id="codicon-').Count -eq 44) -and $spriteContent -match 'id="codicon-discard"' -and $spriteContent -match 'id="codicon-git-commit"' -and $spriteContent -match 'id="codicon-git-branch-compact"' -and $spriteContent -match 'id="codicon-json"' -and $spriteContent -match 'id="codicon-collection"' -and $spriteContent -match 'id="codicon-collection-small"' -and $spriteContent -match 'id="codicon-new-session"' -and $spriteContent -match 'id="codicon-open-preview"' -and $spriteContent -match 'id="codicon-check-compact"' -and $spriteContent -match 'id="codicon-circle-slash-compact"' -and $spriteContent -match 'id="codicon-debug-disconnect-compact"' -and $spriteContent -match 'id="codicon-checklist-compact"' -and $spriteContent -match 'id="codicon-shield-compact"' -and $spriteContent -match 'id="codicon-chevron-right-compact"' -and $spriteContent -match 'id="codicon-chevron-down-compact"' -and $spriteContent -match '1c47ab36a4bb845c437866405c2fa67b8ca0fe36' -and $attributionContent -match 'Creative Commons Attribution 4\.0 International' -and $iconGeneratorContent -match '\.\.\\workbench\\icons' -and $iconGeneratorContent -match 'Get-Content -LiteralPath \$sourcePath -Raw' -and $iconGeneratorContent -notmatch 'Invoke-WebRequest|Invoke-RestMethod|https://raw' -and $launcherContent -match 'workbenchIconSource' -and $launcherContent -match 'Copy-Item -LiteralPath \$workbenchIconSource -Destination \$resolvedSiteDirectory -Recurse -Force' -and $indexContent -notmatch 'cdn\.jsdelivr|data-lucide|lucide\.min\.js' -and $appContent -notmatch 'data-lucide|window\.lucide|refreshIcons' -and $appContent -match 'function icon\(name\)' -and $appContent -match 'icons/codicons\.svg#codicon-\$\{name\}' -and $indexContent -match 'icons/codicons\.svg#codicon-json' -and $indexContent -match 'icons/codicons\.svg#codicon-git-branch-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-check-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-collection-small' -and $indexContent -match 'icons/codicons\.svg#codicon-circle-slash-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-debug-disconnect-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-checklist-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-shield-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-chevron-right-compact' -and $indexContent -match 'icons/codicons\.svg#codicon-chevron-down-compact' -and $indexContent -match 'data-view="catalog"[\s\S]*?codicon-collection' -and $indexContent -match 'data-view="plan"[\s\S]*?codicon-new-session' -and $indexContent -match 'data-view="preview"[\s\S]*?codicon-open-preview' -and $appContent -match 'icon\("discard"\)'
    Add-TestResult -Name 'local-codicon-sprite' -Passed $codiconContractValid -Detail 'The Workbench owns a pinned, attributed, offline Codicon source set and generated sprite, stages it recursively, uses the native commit and discard glyphs, and has no Lucide or runtime-network dependency.'

    $browserContractValid = $indexContent -match '<title>Hosted Copilot Rule Manager</title>' -and $indexContent -match '<h1>HOSTED COPILOT RULE MANAGER</h1>' -and $indexContent -match 'class="title-draft-menu" id="draft-menu"[\s\S]*id="export-button"[\s\S]*for="import-input"[\s\S]*id="close-button"' -and $indexContent -match 'class="ide-statusbar type-compact" aria-label="Workbench status"' -and $indexContent -match 'id="status-snapshot"' -and $indexContent -match 'id="status-mapped"' -and $indexContent -match 'id="status-headroom"' -and $indexContent -notmatch 'id="metrics-band"|Refresh candidates' -and $appContent -match 'indexedDB\.open' -and $appContent -match 'localStorage\.setItem' -and $appContent -match 'hosted-rule-workbench-draft' -and $appContent -match 'elements\["status-snapshot"\]\.textContent' -and $appContent -match 'elements\["status-mapped"\]\.textContent' -and $appContent -notmatch 'elements\["metrics-band"\]\.innerHTML'
    ...
    $planActivityBadgeValid = $indexContent -match 'id="promotion-plan-stage"[^>]*title="Promotion plan, 0 rules"[^>]*aria-label="Promotion plan, 0 rules"' -and $indexContent -match 'class="stage-count-badge" id="plan-activity-count" aria-hidden="true" hidden' -and $appContent -match 'const planCount = getPlanCandidates\(\)\.length' -and $appContent -match 'elements\["plan-count"\]\.textContent = formatNumber\(planCount\)' -and $appContent -match 'planCount > 999 \? "999\+" : formatNumber\(planCount\)' -and $appContent -match 'elements\["plan-activity-count"\]\.hidden = planCount === 0' -and $appContent -match 'Promotion plan, \$\{planCountLabel\}' -and $stylesContent -match '\.stage-count-badge\s*\{[^}]*right:\s*4px;[^}]*bottom:\s*5px;[^}]*min-width:\s*16px;[^}]*height:\s*16px;[^}]*border-radius:\s*999px' -and $stylesContent -match 'html\.theme-hosted-dark body \.stage-count-badge\s*\{[^}]*font-size:\s*10px !important;[^}]*font-weight:\s*600 !important;[^}]*line-height:\s*14px !important'
    ...
    $componentConsistencyValid = $stylesContent -match '#candidate-panel > \.panel-heading\s*\{[^}]*padding-block:\s*8px' -and $stylesContent -match 'html\.theme-hosted-dark :is\(\.status-badge, \.candidate-state, \.candidate-lifecycle, \.decision-badge, \.recommendation-badge, \.catalog-status\)\s*\{[^}]*font-weight:\s*600 !important' -and $stylesContent -match 'html\.theme-hosted-dark body \.plan-table \.candidate-link,\s*html\.theme-hosted-dark body \.preview-raw-payload > summary > span\s*\{[^}]*font-size:\s*var\(--font-size-default\) !important;[^}]*font-weight:\s*600 !important;[^}]*line-height:\s*var\(--line-height-default\) !important' -and $stylesContent -match '\.diff-file-heading\s*\{[^}]*color:\s*#e6edf3;[^}]*border-bottom:\s*1px solid var\(--line\)'
    Add-TestResult -Name 'component-typography-consistency' -Passed $componentConsistencyValid -Detail 'Candidate and assessment list headers align at 48px, semantic pills remain semibold inside compact surfaces, Plan candidate IDs match tree hierarchy, and Preview section labels use one heading treatment.'
    ...
    $codiconShellValid = $indexContent -match 'class="brand-icon codicon"[^>]*>[\s\S]*?icons/codicons\.svg#codicon-json' -and $indexContent -match 'icons/codicons\.svg#codicon-git-branch-compact' -and $indexContent -match 'class="stage-link active clickable"[^>]*title="Eligible candidates"' -and $indexContent -match '<h2 id="candidate-heading">Candidate Sources:</h2>' -and $indexContent -match '<h2 id="assessment-results-heading">Assessment Results:</h2>' -and $indexContent -match '<h2>Promotion Plan:</h2>' -and $indexContent -match '<h2>Promotion Preview:</h2>' -and $stylesContent -match '@media \(min-width: 768px\)[\s\S]*?grid-template-columns:\s*48px minmax\(0, 1fr\);[\s\S]*?grid-template-rows:\s*35px minmax\(0, 1fr\) 22px' -and $stylesContent -match '\.stage-link\.active::before\s*\{[^}]*background:\s*var\(--ink\)' -and $stylesContent -match '\.status-left \.status-item:first-child svg\s*\{[^}]*width:\s*13px;[^}]*height:\s*13px' -and $stylesContent -match '\.workspace-tab\s*\{[^}]*text-transform:\s*uppercase' -and $stylesContent -match '\.candidate-pane-tab\s*\{[^}]*text-transform:\s*uppercase'
    ...
    $previewEditorValid = $indexContent -match '<h2>Promotion Preview:</h2>' -and $indexContent -notmatch 'preview-editor-icon' -and $indexContent -match 'class="preview-sidebar-title">[\s\S]*icons/codicons\.svg#codicon-shield[\s\S]*<span>Approval</span>' -and $stylesContent -match '#preview-view \.page-heading\s*\{[^}]*background:\s*var\(--surface\);[^}]*border:\s*1px solid var\(--line\)' -and $stylesContent -match '#preview-view \.preview-layout\s*\{[^}]*grid-template-columns:\s*280px minmax\(0, 1fr\)' -and $stylesContent -match '#preview-view \.preview-summary\s*\{[^}]*height:\s*100%;[^}]*align-self:\s*stretch;[^}]*background:\s*var\(--surface\)' -and $stylesContent -match '#approval-badge,\s*#preview-summary\s*\{[^}]*display:\s*none' -and $stylesContent -match '#preview-view \.preview-code\s*\{[^}]*background:\s*var\(--paper\);[^}]*border:\s*0'
    ...
    $detailPresentationValid = $detailPresentationValid -and $appContent -match 'if \(!candidate\) \{[\s\S]*?icon\("list-selection"\)[\s\S]*?refreshPresentation\(\);\s*return;'
    ...
    $previewDiffValid = $indexContent -match 'id="preview-diff"' -and $indexContent -match 'id="preview-payload-diff"' -and $indexContent -match '<details class="preview-raw-payload">' -and $indexContent -match 'Raw Selection Payload' -and $appContent -match 'function renderPreviewChanges\(' -and $appContent -match 'decision\.action === "add"' -and $appContent -match 'decision\.action === "retire"' -and $appContent -match 'function diffTextLines\(' -and $appContent -match 'diff-line \$\{line\.type\}' -and $appContent -match 'function renderPayloadChanges\(' -and $appContent -match 'Object\.keys\(after\)\.filter' -and $appContent -match 'renderPayloadColumn\("Default", beforeLines, "delete"\)' -and $appContent -match 'renderPayloadColumn\("Current", afterLines, "add"\)' -and $appContent -match 'class="payload-line-number"' -and $appContent -match 'class="payload-line-marker"' -and $appContent -match 'class="highlight-width-track"' -and $stylesContent -match '\.highlight-width-track\s*\{[^}]*width:\s*max-content;[^}]*min-width:\s*100%' -and $stylesContent -match '\.diff-line\.add\s*\{[^}]*background:\s*rgb\(14, 42, 31\)' -and $stylesContent -match '\.diff-line\.delete\s*\{[^}]*background:\s*rgb\(54, 26, 33\)' -and $stylesContent -match '\.payload-column\.add \.payload-line\s*\{[^}]*background:\s*rgb\(14, 42, 31\)' -and $stylesContent -match '\.payload-column\.delete \.payload-line\s*\{[^}]*background:\s*rgb\(54, 26, 33\)' -and $stylesContent -match '\.payload-columns\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1fr\)'
