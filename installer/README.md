...
| `stage-pending-pr-review.prompt.md` | **BETA: Challenge and stage a pending PR review** | Run after `/code-review-committed-changes`, settle human challenges in the same chat, inspect the exact preview, and explicitly approve creation of one unsubmitted pending review |

> [!WARNING]
> `/stage-pending-pr-review` is in BETA. Inspect every proposed and staged comment before submission. The workflow never submits the review; submission requires a new explicit instruction.

The BETA workflow is: run `/code-review-committed-changes`, challenge or supplement its findings in the same chat, invoke `/stage-pending-pr-review` when the finding set is settled, inspect the exact preview, and explicitly approve only that plan. The workflow then verifies the pending review and returns its URL plus a separate request-changes summary.
