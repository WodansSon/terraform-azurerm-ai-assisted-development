...
| `/update-api-version` | `update-api-version.prompt.md` | Phase-gated ARM API version bump for a Resource Provider (readiness checks, scoped code migration, build validation, and a risk summary), invoked as `/update-api-version {{SERVICE}}/{{FROM_API_VERSION}} {{SERVICE}}/{{TARGET_API_VERSION}}` |
...
| `update-api-version.prompt.md` | **Bump a Resource Provider to a newer ARM API version** through a phase-gated flow | Invoke as `/update-api-version {{SERVICE}}/{{FROM_API_VERSION}} {{SERVICE}}/{{TARGET_API_VERSION}}` to run readiness checks, scoped code migration, build validation, and a migration risk summary while the final risk call and PR submission stay engineer-owned |
