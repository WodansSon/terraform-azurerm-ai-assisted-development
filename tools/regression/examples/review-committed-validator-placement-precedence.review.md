# Code Review: committed validator placement precedence

## CHANGE SUMMARY

- **Files Changed**: 1 modified Go implementation file
- **Branch**: fixture/committed-validator-placement
- **Scope**: reviews schema validation placement in the committed diff
...
The committed change adds readable nested helper composition for `path_match` and a separate anonymous custom parser for `routing_expression`. Under `IMPL-SCHEMA-005`, established helper composition remains inline even when nested; only the genuinely complex parser crosses the service-local validator boundary.

## AZURERM LINTER

- **Status**: No issues
- **Run Scope**: committed Go diff
- **Issue Count**: 0
