...
The benchmark is satisfied when the review surfaces four separate findings from one cold review pass:
...
The benchmark fails if the review leads with secondary polish concerns, collapses distinct issue classes into one summary, or drops the PUT-versus-PATCH observation because stronger issues already exist.

The benchmark also fails if the rendered committed review leaks editor-local or absolute-disk file references such as `vscode-file://`, `workbench.html`, `AppData`, or `workspaceStorage` instead of repo-scoped or PR-scoped paths.
