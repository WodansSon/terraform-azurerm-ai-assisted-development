# Sanitized Fixture: Local Review Catches Embedded Terraform Formatting Drift In Acceptance Tests
...
A local change updates an acceptance-test helper under `internal/services/example/`.

The embedded Terraform configuration in the Go heredoc uses tabs at the start of multiple configuration lines and also mixes tabs and spaces inside one line so the block can look aligned in the editor while still violating the repository formatting rule.
...
- activate the Go and acceptance-test review scope rules
