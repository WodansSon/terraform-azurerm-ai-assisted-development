...
Elsewhere in the same service there are older validators that still use legacy placement, but those files are not part of the current change.

The schema now contains three validation styles:

- a short inline enum validator using `validation.StringInSlice(...)`
- a shared resource-ID validator using `commonids.Validate...`
- a long anonymous inline `ValidateFunc` closure with bespoke string checks for a routing-rule name
...
"routing_rule_name": {
    ...
    ValidateFunc: func(v interface{}, k string) (warnings []string, errors []error) {
        ...
        if strings.HasPrefix(value, "-") {
            errors = append(errors, fmt.Errorf("property `%s` cannot start with `-`", k))
        }
...
- recommend moving the bespoke anonymous closure into a file-specific validator under `validate/`, for example `validate/front_door_custom_domain_id.go`
- recommend the matching unit test file, for example `validate/front_door_custom_domain_id_test.go`
